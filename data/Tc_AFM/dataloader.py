"""Crystal graph dataset for Tc_AFM."""
...
class CIFData(InMemoryDataset):
    ...
    Input: ``root/dataset_with_atom_moments_final.csv``.
    A CSV with a header and the original positional columns for IDs,
    structures, transition temperatures (K), and atom magnetic moments.
    ...
    random_seed : int
        Seed for shuffling input rows before graph construction.
    transform, pre_transform, pre_filter : callable, optional
        Standard PyG access-time transform and processing hooks.

    Graph fields: x, magmon, edge_index, y, displacement, distance, and id.
    ``id`` stores the full material identifier from the input CSV.
    ...
    Processing skips rows with unparseable targets and retains only
    rows whose atom magnetic moments have exactly zero mean.
    """
    def __init__(self, root, max_num_nbr=12, radius=8, neighbor_strategy='k-nearest',
        ...
        id_prop_file = os.path.join(root, 'dataset_with_atom_moments_final.csv')
    ...
    def process(self):
        ...
        for row in tqdm(self.id_prop_data, desc='Processing...'):
            cif_id = row[0]
            try:
                target = ast.literal_eval(row[9])
            except Exception as e:
                print(f"{cif_id} don't have Tc")
                continue
            crystal = Structure(lattice=ast.literal_eval(row[3]),
                                species=ast.literal_eval(row[5]),
                                coords=ast.literal_eval(row[2]),
                                coords_are_cartesian=True)
            magmon = torch.tensor(ast.literal_eval(row[10]), dtype=torch.float)
            if magmon.mean() != 0.0:
                continue
